...
`;
    ...
    if (visualContainer) {
        if (scorePerc >= 50 && imgHigh) {
            ...
            let mascotText = scorePerc >= 80 ? 'Bravo !' : (scorePerc >= 50 ? 'Bien joué !' : 'Essaie encore !');
            visualContainer.innerHTML = `
${mascotText}
${mascotFace}
`;
...
function loadStep(idx) {
   ...
   if(type === 'crossword') {
        if(cwData.length === 0) { stage.insertAdjacentHTML('beforeend', "Usa il Costruttore Cruciverba (🧩)."); return; }
...
`);
        const imgContent = dbImg[word] ? `` : '📷 Immagine (Opzionale)';
        stage.insertAdjacentHTML('beforeend', `
