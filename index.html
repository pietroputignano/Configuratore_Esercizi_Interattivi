...
<head>
    ...
    <!-- IL NOSTRO NUOVO FILE DEGLI STILI -->
    <link rel="stylesheet" href="style.css">
</head>
<body class="p-2 md:p-4 flex justify-center items-center min-h-screen">

    <!-- MODAL MAPPER -->
    <div id="mapper-modal" class="fixed inset-0 hidden flex items-center justify-center p-4 modal-bg z-[100]">
    ...
    <!-- MODAL CRUCIVERBA -->
    <div id="cw-modal" class="fixed inset-0 hidden flex items-center justify-center p-4 modal-bg z-[100]">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[95vh]">
            <div class="p-4 border-b flex justify-between items-center bg-orange-50">
                ...
                <div class="flex gap-2">
                    <button onclick="findCWWords()" class="bg-blue-500 text-white px-4 py-2 rounded-full font-bold text-sm">🔍 Genera Definizioni</button>
                    <button onclick="closeCWBuilder()" class="bg-orange-600 text-white px-6 py-2 rounded-full font-bold text-sm">Salva Griglia</button>
                </div>
            </div>
            <div class="p-3 bg-gray-50 border-b flex flex-wrap gap-4 items-center">
                <label class="flex items-center gap-2 text-xs font-bold text-blue-900">
                    <input type="checkbox" id="cw-in-show-clues" checked> Mostra Definizioni
                </label>
                <input id="cw-in-verb" type="text" class="border border-gray-300 p-2 rounded text-xs w-64 shadow-inner" placeholder="Etichetta / Verbo (es: HABITER)">
            </div>
            <div class="flex-grow overflow-auto p-6 bg-slate-200 flex flex-col items-center">
                <div id="cw-builder-grid" class="cw-grid bg-white p-4 shadow-lg rounded-lg mb-6"></div>
                <div id="cw-clues-container" class="w-full bg-white p-4 shadow-lg rounded-lg hidden"></div>
            </div>
        </div>
    </div>
    
    <!-- MODAL REBUS -->
    <div id="rebus-modal" class="fixed inset-0 hidden flex items-center justify-center p-4 modal-bg z-[100]">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[90vh]">
            <div class="p-4 border-b flex justify-between items-center bg-green-50">
                <h3 class="font-bold text-green-900">📝 Costruttore Frasi (Rebus)</h3>
                <button onclick="closeRebusBuilder()" class="bg-green-600 text-white px-6 py-2 rounded-full font-bold text-sm">Salva Frasi</button>
            </div>
    ...
    <!-- MAIN APP -->
    <div class="w-full max-w-[1600px] flex flex-col xl:flex-row gap-6">
        
        <!-- PANNELLO DI SINISTRA (CONFIGURATORE) -->
            ...
            <div class="space-y-6">
                ...
                <div class="p-4 bg-blue-50 rounded-xl border border-blue-200 space-y-3">
                    ...
                    <div class="mt-3 pt-3 border-t border-blue-200">
                        <p class="font-bold uppercase mb-1 text-blue-800 text-[10px]">Testo Lettura Completa (Opzionale)</p>
                        <input id="tts-context-f" type="text" class="w-full p-2 border rounded text-xs mb-2" placeholder="es: J'habite. Tu habites..." oninput="initGame(); startGame();">
                ...
                <div class="p-4 bg-blue-900 rounded-xl border border-blue-900 space-y-3 shadow-inner">
                    ...
                    <div class="mt-3 pt-3 border-t border-blue-800">
                        <p class="font-bold uppercase mb-1 text-white text-[10px]">Testo Lettura Completa (Opzionale)</p>
                        <input id="tts-context-d" type="text" class="w-full p-2 border rounded text-xs mb-2 text-gray-800" placeholder="es: J'habite. Tu habites..." oninput="initGame(); startGame();">
                        <p class="font-bold uppercase mb-1 text-white text-[10px]">Dati Livello DIFFICILE (separati da ;)</p>
                        <textarea id="items-list-d" class="w-full h-16 p-2 border rounded text-xs bg-slate-100 placeholder-gray-400 text-gray-800" placeholder="habite; habites; habitons; habitez" oninput="initGame(); startGame();"></textarea>
                    </div>
                </div>

                <!-- CARICA AUDIO PER STEP -->
                ...
                <!-- NOME FILE ESPORTATO -->
                <div class="p-3 bg-emerald-50 rounded-lg border border-emerald-200 mt-4">
                    <p class="font-bold text-emerald-900 uppercase mb-2 text-[10px]">📥 Nome File Export</p>
                    <input id="in-filename" type="text" class="w-full p-2 border rounded text-xs" placeholder="Verrà usato il Titre se lasci vuoto">
                    <p class="text-[8px] text-gray-500 mt-1 italic">Nome del file .zip che verrà scaricato (senza estensione).</p>
                </div>
            </div>
        ...
        <!-- PANNELLO DI DESTRA (ANTEPRIMA GIOCO) -->
    ...
    <script>
        ...
        function validate(correct) {
            ...
            if(correct) { 
                ...
                setTimeout(() => { 
                    if (['domino','autocollantes','crossword','dressing'].includes(type) || curStep >= items.length - 1) {
                        showEndScreen();
                    } else {
                        loadStep(curStep + 1);
                    }
    ...
    <div class="w-full max-w-5xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col min-h-[750px] relative border border-gray-200">
        ...
        function playConsigne() { const key = curLvl === 'facile' ? 'aud-f' : 'aud-d'; if(GAME_CONFIG.audio[key]) new Audio(GAME_CONFIG.audio[key]).play(); else { const m = new SpeechSynthesisUtterance(GAME_CONFIG.levels[curLvl].consigne); m.lang='fr-FR'; m.rate=0.6; window.speechSynthesis.speak(m); } }
    </` + `script>
</body>
</html>`;
